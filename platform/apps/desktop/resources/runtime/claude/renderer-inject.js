((cssText, artDataUrl, themeConfig) => {
  const STATE_KEY = "__CLAUDE_DREAM_SKIN_STATE__";
  const DISABLED_KEY = "__CLAUDE_DREAM_SKIN_DISABLED__";
  const STYLE_ID = "claude-dream-skin-style";
  const CHROME_ID = "claude-dream-skin-chrome";
  ...
  const VERSION = __CLAUDE_DREAM_SKIN_VERSION_JSON__;
  const STYLE_REVISION = __CLAUDE_DREAM_SKIN_STYLE_REVISION_JSON__;
  const PAYLOAD_REVISION = __CLAUDE_DREAM_SKIN_PAYLOAD_REVISION_JSON__;
  const THEME = themeConfig && typeof themeConfig === "object" ? themeConfig : {};
  const ART = THEME.art && typeof THEME.art === "object" ? THEME.art : {};
  const ART_METADATA = THEME.artMetadata && typeof THEME.artMetadata === "object"
    ? THEME.artMetadata : null;
  const ANALYSIS_CACHE_KEY = "__CLAUDE_DREAM_SKIN_ANALYSIS_CACHE__";
  ...
  const detectShellMode = () => {
    ...
    // Claude Desktop may use class-based theming
    ...
    // Temporarily remove skin class to sample native scheme
    try {
      const hadSkin = root.classList.contains("claude-dream-skin");
      const savedShell = root.getAttribute(SHELL_ATTR);
      samplingNativeShell = true;
      if (hadSkin) root.classList.remove("claude-dream-skin");
      if (savedShell !== null) root.removeAttribute(SHELL_ATTR);
      let colorScheme = "";
      try {
        colorScheme = getComputedStyle(root).colorScheme || "";
      } finally {
        if (hadSkin) root.classList.add("claude-dream-skin");
    ...
    if (!root.classList.contains("claude-dream-skin")) {
      const samples = [
        body,
        document.querySelector("main"),
        document.querySelector("[data-sidebar-state]"),
      ].filter(Boolean);
      let votesLight = 0;
      let votesDark = 0;
  ...
  const applyTheme = (root, shell) => {
    ...
    setStyleProperty(root, "--dream-skin-name", cssString(THEME.name || "Claude Dream Skin"));
    setStyleProperty(root, "--dream-skin-tagline", cssString(THEME.tagline || "Make something wonderful."));
    setStyleProperty(root, "--dream-skin-project-prefix", cssString(THEME.projectPrefix || ""));
    setStyleProperty(root, "--dream-skin-project-label", cssString(THEME.projectLabel || ""));
  };
  ...
  const applyRootState = (root) => {
    ...
    root.classList.add("claude-dream-skin");
    return shell;
  };
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    // Claude Desktop: main content area
    const mainArea = document.querySelector("main") || document.querySelector("[role='main']");

    // Detect the home/welcome page (no conversation loaded)
    const composerEmpty = document.querySelector("[data-placeholder]");
    const messageList = document.querySelector("[data-test-render-count]") ||
      document.querySelector("[data-testid*='message']") ||
      document.querySelector("article");
    const isHome = !messageList && composerEmpty;

    for (const candidate of document.querySelectorAll(".dream-skin-home")) {
      if (!isHome || candidate !== (mainArea?.parentElement ?? mainArea)) {
        candidate.classList.remove("dream-skin-home");
      }
    }
    if (isHome && mainArea) {
      (mainArea.parentElement ?? mainArea).classList.add("dream-skin-home");
    }

    // Tag overlay surfaces for consistent styling
    ...
    if (!mainArea || !document.body) return;
    if (observedMainArea !== mainArea) {
      resizeObserver?.disconnect();
      resizeObserver?.observe(mainArea);
      observedMainArea = mainArea;
      layout = true;
    }
    ...
    setTextContent(chromeParts.name, THEME.name || "Claude Dream Skin");
    setTextContent(chromeParts.subtitle, THEME.brandSubtitle || "CLAUDE DREAM SKIN");
    ...
    if (layout || created) {
      ...
      const mainBox = mainArea.getBoundingClientRect();
      setStyleProperty(chrome, "left", `${Math.round(mainBox.left)}px`);
      setStyleProperty(chrome, "top", `${Math.round(mainBox.top)}px`);
      setStyleProperty(chrome, "width", `${Math.round(mainBox.width)}px`);
      setStyleProperty(chrome, "height", `${Math.round(mainBox.height)}px`);
    }
    chrome.classList.toggle("dream-skin-home-shell", Boolean(isHome));
  ...
  const cleanup = () => {
    ...
    document.documentElement?.classList.remove("claude-dream-skin");
...
})(__CLAUDE_DREAM_SKIN_CSS_JSON__, __CLAUDE_DREAM_SKIN_ART_JSON__, __CLAUDE_DREAM_SKIN_THEME_JSON__)
